...
app.factory('BlogService',['$http', '$q','$rootScope', function($http,$q,$rootScope){
	...
	var BASE_URL="http://localhost:8083/CollaborationBackEnd"
		...
		commentBlog: function(blogcomment,id,comment){
			console.log("Your comment is "+blogcomment.bcomment)
			return $http.post(BASE_URL+'/blogcomment/'+id+'/'+comment,blogcomment)
			.then(function(response){
				return response.data;
			},
			function(errResponse){
		...
		addrating: function(){
			console.log("Your rating is"+blogcomment.rating)
			return $http.post(BASE_URL+'/blograting',blogcomment.rating,blogcomment)
			.then(function(response){
				return response.data;
			},
			function(errResponse){
				console.error('Error while rating the post');
				return $q.reject(errResponse);
			});
		},
